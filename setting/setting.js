                ...
                bootbox.prompt({
                    ...
                    callback: function(newScriptName) {
                        ...
                        if(newScriptName == null ){
                            ...
                            || newScriptName.indexOf(",")>=0|| newScriptName.indexOf(".")>=0 || newScriptName.indexOf(";")>=0 || newScriptName.indexOf(" ")>=0){
                            scriptNameOK = false;
                        }else {
...
function initFriendButton(scriptValue){
    ...
    $("#selectFriendItem"+currentId).change(function(){
        if($(this).val()!=-1){
            ...
            $("#selectFriendItemImg"+currentId).removeAttr('src').replaceWith($("#selectFriendItemImg"+currentId).clone());;
        }
    });
    ...
    if(scriptValue.length > 4){
        $("#selectFriendOnlyFriend"+currentId).val(scriptValue[4]).trigger('change');
    }
    if(scriptValue.length > 5){
        $("#selectFriendScrollCnt"+currentId).val(scriptValue[5]).trigger('change');        
    }
}
...
function getCurrentScript(){
    ...
    $("#skill-list").children().each(function(){
                ...
                switch(itemTitle){
                    ...
                    break;
                    case "抽箱":
                    newScript+="getBox("+$("#getBoxReset"+itemId).val()+","+$("#getBoxFast"+itemId).val()+");";
                    break;
                    case "選擇關卡":
                    ...
                    newScript+='selectFriend('+filter+',"'+servant+'","'+item+'",'+$("#selectFriendItemFull"+itemId).val()
                        +','+$("#selectFriendOnlyFriend"+itemId).val()+','+$("#selectFriendScrollCnt"+itemId).val()+');';
                    break;
                    case "選擇隊伍":
                    ...
                    break;
                    case "設定技能改變寶具顏色":
                    newScript+="setSpaceUltColor("+$("#spaceUltColor"+itemId).val()+");";
                    break;
                    case "使用技能":
...
function initHTML(result){
    ...
    if(server == "JP"){
        $("#serverMessage").text("日服 "+version+" 啟動成功 - 其他伺服器版本請至腳本商店下載");
    }else if(server == "TW"){
        $("#serverMessage").text("台服 "+version+" 啟動成功 - 其他伺服器版本請至腳本商店下載");
    }
    ...
    bootbox.prompt(
            {
                ...
                callback: function(imageName) {
                    ...
                    if(imageName.length <= 0 || imageName.indexOf(",")>=0 || imageName.indexOf(".")>=0 || imageName.indexOf(";")>=0 || imageName.indexOf(" ")>=0){
                        imageNameOK = false;
                    }
                    if(imageNameOK){
...
function resetScript(result){
    ...
    scriptContentList.forEach(function(content){
        ...
        if(checkstring(content,"selectStage")){
            ...
            $("#getBoxReset"+commandId).val(scriptValue[0]).trigger('change');
            ...
            if(scriptValue.length >= 2){
                $("#getBoxFast"+commandId).val(scriptValue[1]).trigger('change');
            }
            ...
            $("#spaceUltColor"+commandId).val(content).trigger('change');
